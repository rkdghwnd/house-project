...
const SearchReviewResult = () => {
  ...
  const searchReviewProducts = useSelector(
    (state: RootState) => state.search.searchReviewProducts
  );
  ...
  const openReviewWritingModal = useCallback(
    (id: number, image_url: string, brand_name: string, product_name: string) =>
      () => {
        // 리뷰작성 모달 열기
        dispatch(
