...
import { UserSliceStateType } from '../types/stateTypes';
...
const initialState: UserSliceStateType = {
  ...
  scrapBook: { name: '', profile_image: '', scrap: [] },
...
const userSlice = createSlice({
  ...
  reducers: {
    ...
    addLikersInMyLiked(state, action) {
      const findedLikers = state.myLiked.liked.find(
        (el) => el.id === action.payload.ReviewId
      );
      if (!findedLikers || !findedLikers.Likers) {
        return;
      }

      const reviewLikers: { id: number }[] = findedLikers.Likers;
    ...
    removeLikersInMyLiked(state, action) {
      ...
      const reviewLikers: { id: number }[] = findedLikers.Likers;
      findedLikers.Likers = reviewLikers.filter(
