import React, { useCallback, forwardRef, MouseEvent, LegacyRef } from 'react';
import { useSelector } from 'react-redux';
import { RootState } from '../../reducers';

const ProductShipment = (props: {}, ref: LegacyRef<HTMLDivElement>) => {
  const productions = useSelector(
    (state: RootState) => state.productions.productions
  );
  const openShipment = useCallback((e: MouseEvent<HTMLButtonElement>) => {
    if (!e.currentTarget.parentNode?.parentNode) {
      return;
    }
    (e.currentTarget.parentNode.parentNode as HTMLBaseElement).classList.add(
      'is-open'
    );
  }, []);
  const {
